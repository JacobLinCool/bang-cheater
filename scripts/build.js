...
writeFileSync(resolve(dist, "bang-cheater.user.js"), header + "\n\n" + bundled);
writeFileSync(resolve(dist, "bang-cheater.min.user.js"), header + "\n\n" + minified);
process.stdout.write("Done\n");

console.log("----------------------------------------");
console.log(
    `Bundled: ${(statSync(resolve(dist, "bang-cheater.user.js")).size / 1024).toFixed(1)} KB`,
);
console.log(
    `Minified: ${(statSync(resolve(dist, "bang-cheater.min.user.js")).size / 1024).toFixed(1)} KB`,
);

function build() {
    const bundled = run("npx esbuild --bundle --target=es6 src/index.js");
    if (bundled.status !== 0) {
        console.error("\n\n" + bundled.stderr.toString());
        process.exit(1);
    }
    const minified = run("npx esbuild --bundle --minify --target=es6 src/index.js");
...
function make_header() {
    ...
    const metadata = {
        ...
        downloadURL:
            "https://github.com/JacobLinCool/bang-cheater/raw/dist/bang-cheater.min.user.js",
